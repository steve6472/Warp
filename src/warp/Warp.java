...
import warp.commands.misc.*;
import warp.commands.warp.*;
...
public class Warp extends JavaPlugin implements Listener
{
	...
	public void onEnable()
	{
		...
		registry = new CustomCommandRegistry(this);
		...
		new DummyCommand(this);
		new MSPTCommand(this);
		new MSPTDisplayCommand(this);

//		new DestroyCommand(this);
//		new ForceEnchantCommand(this);
//		new LoreCommand(this);
//		new WorldCommand(this);
//		new ModifyItemCommand(this);
//		new NoClipCommand(this);
	...
	public void printAllWarps(CommandContext<CommandListenerWrapper> context) throws CommandSyntaxException
	{
		if (warps.isEmpty())
		{
	...
	public void sayCreator(CommandContext<CommandListenerWrapper> context, String uuid)
	{
		...
		if (player == null)
		{
			context.getSource().getBukkitSender().sendMessage(RED + "Creator is offline but his UUID is " + YELLOW + uuid);
		} else
		{
			context.getSource().getBukkitSender().sendMessage(RED + "Creator: " + WHITE + player.getDisplayName());
		}
	}
